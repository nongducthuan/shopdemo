        ...
        {banners.map((b) => (
          <div key={b.id} className="col-md-3 col-6 mb-3">
            <div className="card shadow-sm">
              {b.image_url && <img src={`${backendUrl}${b.image_url}`} className="card-img-top" alt={b.title} />}
              <div className="card-body">
                <h5>{b.title}</h5>
                <p>{b.subtitle}</p>
                <div className="text-end">
                  <button onClick={() => handleEditBanner(b)} className="btn btn-warning btn-sm me-2">
                    <i className="fas fa-pen"></i>
                  </button>
                  <button onClick={() => handleDeleteBanner(b.id)} className="btn btn-danger btn-sm">
